...
upstream            = None
dataset             = None
external_pred_file  = None   # optional path to CSV/Excel with external predictions
...
If external_pred_file is set, hard predictions are read from that file
(simulating a VEGA model providing only a class label), the same pattern
used for VEGA models in the paper. Otherwise the internal base model is used
for both approaches.
"""))
...
# Resolve hard predictions: external file or internal model
if external_pred_file is not None and Path(external_pred_file).exists():
    display(Markdown(f"- Loading external hard predictions from: {external_pred_file}"))
    ...
    if df_ext[external_pred_col].dtype == object:
        df_ext["_label"] = df_ext[external_pred_col].map(class_to_int)
    else:
        df_ext["_label"] = df_ext[external_pred_col].astype(int)
    # Align with cal_df and test_df by Smiles
    cal_hard_map  = df_ext.set_index("Smiles")["_label"]
    y_cal_hard  = cal_df["Smiles"].map(cal_hard_map).fillna(
        pd.Series(y_cal_pred, index=cal_df.index)).astype(int).values
    y_test_hard = test_df["Smiles"].map(cal_hard_map).fillna(
        pd.Series(y_test_pred, index=test_df.index)).astype(int).values
    display(Markdown("- External hard labels loaded for calibration and test sets."))
else:
    display(Markdown("- No external_pred_file. Using internal model hard labels "
                     "(demonstrates the code path; same predictions as Approach A base model)."))
    y_cal_hard  = y_cal_pred
...
# ==============================================================================
# S9  Coverage guarantee sweep across alpha
# ==============================================================================
